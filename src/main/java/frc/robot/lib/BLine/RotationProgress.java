...
/**
 * Ordered geometric heading schedule, independent of the translation target cursor.
 *
 * <p>A handoff only authorizes looking at the next connected segment. When the nearer
 * projection changes coordinate systems at a cut corner, the remaining interpolation
 * starts at the heading the old coordinate system requested at that same robot position.
 * This preserves the angle, not its derivative; the drive controller still applies PID
 * and angular slew limits. Distances are metres and unwrapped angles are radians.
 */
