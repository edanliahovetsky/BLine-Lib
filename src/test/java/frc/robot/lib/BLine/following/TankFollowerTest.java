...
class TankFollowerTest {
    ...
    @Test void arrivalUsesPoseToleranceWithoutWaitingForMeasuredSpeedOrResettingPidState() {
        for (DriveType type : DriveType.values()) {
            ...
            Follower follower = new Follower(new Path(new Path.Waypoint(new Pose2d(2, 0, Rotation2d.fromDegrees(90)))),
                new FollowerSession.Config(type, () -> rig.pose, value -> rig.pose = value,
                    () -> new ChassisVelocities(.4, 0, .3), value -> rig.output = value,
                    translation, rotation, crossTrack), null, TankFollowerTest::noEvents);
            follower.initialize();
            rig.seconds += .02;
            follower.execute();
    ...
    private static FollowerSession.EventRun noEvents() {
        return new FollowerSession.EventRun() {
    ...
    private static final class Rig {
        ...
        Follower follower(Path path) {
            ...
            return new Follower(path, new FollowerSession.Config(DriveType.TANK, () -> pose, value -> pose = value,
                () -> output, value -> output = value, new PIDController(1.3, 0, 0),
                new PIDController(2, 0, 0.25), new PIDController(0.5, 0, 0)), () -> false, TankFollowerTest::noEvents);
        }
